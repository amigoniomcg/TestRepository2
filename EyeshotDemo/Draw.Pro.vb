...
Partial Class Draw
	...
	Public Shared Sub MotherBoard(viewportLayout As ViewportLayout)
        Dim objStream As Stream = Nothing, mtlStream As Stream = Nothing
        Dim texStreams As Dictionary(Of String, Stream) = Nothing
        devDept.DataSet.DataSet.GetMotherBoardStream(objStream, mtlStream, texStreams)

		Dim ro As New devDept.Eyeshot.Translators.ReadOBJ(objStream, mtlStream, texStreams, Mesh.edgeStyleType.Sharp)

		viewportLayout.StartWork(ro)

	End Sub

	Public Shared Sub Locomotive(viewportLayout As ViewportLayout)

		Dim r1 As Region = Region.CreateRectangle(110, 38)
		#If NURBS Then
		Dim r2 As Region = Region.CreateEllipse(0, 19, 8, 19)
		#Else
		...
		viewportLayout.Entities.Add(u9, Color.IndianRed)

	End Sub

	Public Shared Sub Bunny(viewportLayout As ViewportLayout)

		Dim readAsc As New ReadASC(devDept.DataSet.DataSet.GetStream("bunny.asc"))
		readAsc.DoWork()

		' scales file contents by 100
		For Each entity As Entity In readAsc.Entities

			entity.Scale(100, 100, 100)
		Next

		readAsc.AddToScene(viewportLayout)

		If viewportLayout.Entities.Count > 0 AndAlso TypeOf viewportLayout.Entities(0) Is FastPointCloud Then

			Dim fpc As FastPointCloud = DirectCast(viewportLayout.Entities(0), FastPointCloud)
			...
			viewportLayout.StartWork(bp)
		End If


	End Sub

    Public Shared Sub Pocket(viewportLayout As ViewportLayout)

        Dim pts As Point2D() = New Point2D() {New Point2D(0, 0), New Point2D(40, 0), New Point2D(40, 20), New Point2D(60, 20), New Point2D(60, 10), New Point2D(100, 10), _
            New Point2D(100, 60), New Point2D(60, 60), New Point2D(60, 30), New Point2D(40, 30), New Point2D(40, 80), New Point2D(0, 80), _
            New Point2D(0, 0)}
        ...
        viewportLayout.Entities.Add(outerContour, Color.OrangeRed)
        ...
        viewportLayout.Entities.Add(innerContour, Color.OrangeRed)
        ...
        For i As Integer = 1 To stepCount - 1
            For Each crv As Entity In passes
                ...
                viewportLayout.Entities.Add(en, Color.DarkBlue)
            Next
        Next
    End Sub

    Public Shared Sub Primitives(viewportLayout As ViewportLayout)

        viewportLayout.Grid.[Step] = 25
		viewportLayout.Grid.Min.X = -25
		viewportLayout.Grid.Min.Y = -25
        viewportLayout.Grid.Max.X = 125
		viewportLayout.Grid.Max.Y = 175
		...
		viewportLayout.Entities.Add(mesh__1, Color.GreenYellow)
		offsetX += deltaOffset
		...
		viewportLayout.Entities.Add(lp, Draw.Color)
		offsetX += deltaOffset
	...
	Public Shared Sub TerrainTriangulation(viewportLayout As ViewportLayout)
        
        Form1.SetBackgroundStyleAndColor(viewportLayout)
        ...
        viewportLayout.Entities.Add(m)

        Dim pln As New Plane(New Point3D(0, 20, 20), New Vector3D(20, -30, 10))
        
        Dim pe As New PlanarEntity(pln, 25)
        
        viewportLayout.Entities.Add(pe, Color.Magenta)
        
        Dim curves As ICurve() = m.Section(pln, 0)
        
        For Each ent As Entity In curves
           viewportLayout.Entities.Add(ent)
        Next

	End Sub

	Public Shared Sub CompositeCurveMeshing(viewportLayout As ViewportLayout)

        Form1.SetDisplayMode(viewportLayout, displayType.Shaded)
        ...
        viewportLayout.Entities.Add(m, Color.Salmon)
