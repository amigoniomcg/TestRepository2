...
Partial Class Draw
	...
	Public Shared Sub HairDryer(viewportLayout As ViewportLayout)

		Dim bhd As New BuildHairDryer()
		viewportLayout.StartWork(bhd)
    ...
    Private Class BuildHairDryer
        ...
        Protected Overrides Sub DoWork(worker As System.ComponentModel.BackgroundWorker, doWorkEventArgs As System.ComponentModel.DoWorkEventArgs)
            ...
            ' rear fillet
            Surface.Fillet(New Surface() {s2}, New Surface() {s6}, 10, filletTol, True, True, _
            ...
            ' along handle fillet
            Surface.Fillet(New Surface() {s3}, New Surface() {s2, s6, f2(0)}, 5, filletTol, True, True, _
                True, True, True, False, f3)
            whiteEntList.AddRange(f3)

            ' handle-body fillet
            Surface.Fillet(New Surface() {s1}, New Surface() {s3, s2, s6, f2(0), f3(0)}, 10, filletTol, False, False, _
        ...
        Protected Overrides Sub WorkCompleted(viewportLayout As ViewportLayout)
            viewportLayout.Entities.AddRange(whiteEntList, "Default", Color.WhiteSmoke)
            viewportLayout.Entities.AddRange(darkEntList, "Default", Color.FromArgb(31, 31, 31))
            viewportLayout.Entities.AddRange(offsetEntList, "Default", Color.DarkGray)
    ...
    Public Shared Sub Toolpath(viewportLayout As ViewportLayout)
		...
		viewportLayout.Entities.Add(loft, Color.FromArgb(200, Color.OrangeRed))

		viewportLayout.ZoomFit()

		Dim btp As New BuildToolpath(loft, 0.01, 5)
		viewportLayout.StartWork(btp)
    ...
    Private Class BuildToolpath
        ...
        Protected Overrides Sub WorkCompleted(viewportLayout As ViewportLayout)
            ...
            viewportLayout.Entities.Add(toolPath, "Default", Color.DarkBlue)
            ...
            viewportLayout.Blocks.Add(b1)

            '#End Region

            ' Adds a reference to the tool symbol
            viewportLayout.Entities.Add(New BlockReference(toolPath.Vertices(toolPath.Vertices.Length - 1), "ballTool", 1, 1, 1, 0))
    ...
    Public Shared Sub Flange(viewportLayout As ViewportLayout)
        ...
        Dim rev1 As Solid3D = reg.RevolveAsSolid3D(Math.PI * 2, Vector3D.AxisZ, Point3D.Origin)
        
        viewportLayout.Entities.Add(rev1, System.Drawing.Color.Aqua)
...
Public Shared Sub Bracket(viewportLayout As ViewportLayout)
        ...
        Dim ext1 As Solid3D = r1.ExtrudeAsSolid3D(-4)
        
        viewportLayout.Entities.Add(ext1, Color.YellowGreen)
        ...
        Dim r2 As Region = cc1.OffsetToRegion(-2.5, 0, False)
